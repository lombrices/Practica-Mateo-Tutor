...
class Respuestaincorrecta:
    def __init__(self, contentCodeExcercise, answer, sympyAnswer, frecuency, students):
...
def searchWrongAnswer(listaRespuestas, respuestaIncorrecta):
    ...
    try:
        l=False
        while(i<largo and l==False):
            arrayRespuestas=listaRespuestas[i].sympyAnswer
            l=True
            for j in range(0,len(arrayRespuestas)):
                exp1=arrayRespuestas[j]
                exp2=respuestaIncorrecta[j]
                
                if (exp1.equals(exp2)==False):
                    l=False
            if l==True:
                break
            i=i+1
...
with open('data-1734621405509.csv', newline='') as csvfile:
            ...
            if isCorrect==0 :
                ...
                try:
                    ...
                    for k in range(0,len(respuesta)):
                        respuestaNormalizada=normalize_expression(respuesta[k])
                        respuestaNormalizada=sympify(respuestaNormalizada)
                        respuestasNormalizadas.append(respuestaNormalizada)
                except Exception as e:
                    respuestasNormalizadas=None
                ...
                if (exerciseIndex)==-1 :
                    ...
                    wrongAnswer=Respuestaincorrecta(contentCode,respuesta,respuestasNormalizadas, 1,[int(row[6])])
                ...
                if (stepIndex==-1):
                    ...
                    continue
                wrongAnswerIndex=searchWrongAnswer(ejercicios[exerciseIndex].steps[stepIndex].wrongAnswers, respuestasNormalizadas)
